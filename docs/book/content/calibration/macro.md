...
The model's initial period is **2025** (`start_year`), a calibration decision rather than a default: the start year should be the most recent year the calibration can *observe*, not a projection year. 2025 is the last complete year, and the anchors line up with it directly — the remittance level is the observed 2025 personal-remittance ratio, the sovereign-rate anchor uses FY 2024–2025 interest actuals, and `initial_debt_ratio` $= 0.60$ matches the debt ratio at the *start* of 2025 (end-2024: 60.7%, BTr), where a 2026 start would require ~0.62. The remaining data vintages (OECD taxes 2023, PWT capital 2023, FIES incomes 2023) sit one year closer to the start than they would under a 2026 start. The demographic arrays regenerate for the chosen start year via `ogphl.update_baseline_demographics`, which also rebuilds the demographics-derived remittance parameters.

### Productivity growth

As the default rate of labor augmenting technological change, $g_y$, we use 3.71% — the average annual growth rate of World Bank GDP per capita (constant 2015 US$) for the Philippines over 2000–2019, computed by `ogphl.macro_params` and baked into the packaged parameters. The pre-pandemic window is used deliberately to avoid COVID-era volatility distorting the steady-state productivity target.
...
**The transition follows the government's own consolidation schedule.** A single flat $\alpha_G$ at the identity value would make the model consolidate *immediately* — a primary balance of $-0.7\%$ from the first period, years ahead of the government's plan. Instead, $\alpha_G$ is a declining path pinned to the Medium-Term Fiscal Framework: $[0.1101, 0.1074, 0.1045]$ for 2025, 2026, and 2027 onward. The first two values are the spending shares that reproduce the MTFF's programmed primary balance (the DBCC deficit path less programmed interest: $-2.51$ for 2025, $-2.22$ for 2026) given the model's realized transition revenue under the data-anchored initial wealth; the solved baseline delivers $-2.51$ and $-2.20$ — the 2025 stance exactly on program. The path reaches the debt-stabilizing level in 2027 and runs 0.6–1.3pp looser than the program in 2027–28: with honest initial wealth, transition revenue dips about 1.7pp of GDP below its steady-state share mid-window, and matching the program's 2028 balance exactly would require cutting $\alpha_G$ below its long-run level and snapping back when the program ends — a V-shaped spending path that would over-fit the enforcement. The MTFF program converging toward the model's debt-stabilizing stance by decade's end remains the fiscal block's broadest validation: the steady state is where the government's plan is headed.
...
This calibration instead anchors initial wealth to data with `initial_wealth_ratio` ([OG-Core PR #1189](https://github.com/PSLmodels/OG-Core/pull/1189), **which this calibration requires**). Because initial wealth is a predetermined state, the anchor must be static within the solve, so the parameter is defined against the one aggregate the model has pinned down exactly before the transition runs: $B(0) = $ `initial_wealth_ratio` $\times \bar{Y}_{ss}$, with the value $2.677$ chosen so the *solved* initial wealth-to-GDP ratio equals the data target of 3.35 (delivered: 3.35, reported in the fit table; one solve iteration pins the mapping). Reform runs ignore the parameter and clone the baseline's initial wealth outright — the initial state is history, and policy cannot change what households start with. The data target's construction, each piece from its most authoritative source:

| Piece | Value | Source |
|---|---|---|
| Total capital-output ratio | 3.97 | PWT 11, 2023, current-PPP series pair |
| less public capital stock | −0.38 | IMF ICSD/PIMA: Philippine public capital low by EM standards (~0.35–0.40 of GDP) |
| × domestically-owned share | × 0.80 | BSP IIP: ~20% of capital foreign-owned |
| + domestically-held government debt | +0.48 | BTr: 80% domestic share × 0.60 debt ratio |
| **Household wealth to GDP, 2026** | **≈ 3.35** | |

The capital-side construction is the model-consistent measure — the model forces $B(0) = K_d(0) + D_d(0)$ — and household-net-worth estimates (UBS Global Wealth Report class, ~2.3–2.6× GDP) sit below it, as expected where surveys undercount real property. The initial ratio of 3.35 against a steady-state wealth ratio of 3.98 means Philippine households start *below* their long-run wealth — a converging, fast-growing economy, which is the right shape.

**A documented tension.** With initial wealth at its measured 3.35, the model's *initial-period* foreign-owned capital share comes out near 36% against the ~20% the BSP IIP records: the model demands more capital at the world interest rate ($K(0)/Y \approx 4.5$) than the PWT measures ($\approx 3.97$) — the family-wide $K/Y$ overshoot — so foreign capital fills a larger share of a larger stock. Given that overshoot, the measured household-wealth level and the measured foreign share cannot both be hit; this calibration keeps the household-wealth anchor (it is the quantity the parameter *is*) and records the initial foreign share as a known, mechanically explained miss. Delivering the IIP share at $t=0$ instead would require initial wealth near 4.3× GDP, well above any measured household-claims construction. Under the anchored value the artifact is gone: the 2026→2027 consumption jump is a few percent rather than 41%, investment never collapses, and the transition's fiscal paths are driven by fiscal parameters rather than a phantom windfall. One known residual: the anchor scales the steady-state *age profile* of wealth uniformly (ogcore offers no initial wealth-by-age input), and the packaged `initial_Kg_ratio` of 0.2 predates the ICSD-informed ~0.38 used in the subtraction above — both are flagged for future refinement.
...
---
Baseline model transition paths against Bureau of the Treasury actuals (2022–2024) and the DBCC Medium-Term Fiscal Framework 2022–2030 Midterm Update (October 2025) program. Public investment sits on the MTFF infrastructure program. Revenue runs 1–3pp below the program's general-government equivalent (MTFF national-government revenue plus a +3.78pp bridge for social security contributions and local taxes, from the 2023 OECD-vs-BTr gap) for two documented reasons: the program embeds tax-administration gains the calibration holds constant, and transition revenue sits below its steady-state share while household wealth converges up from its data-anchored initial level. The primary balance reproduces the programmed consolidation where the program is data (2025: −2.51 model vs −2.51 programmed — exact; 2026: −2.20 vs −2.22) and runs 0.6–1.3pp looser in 2027–28 (see the government expenditures section). The debt ratio stays within about ±5pp of the 0.60 anchor along the whole path — easing to ~59% through the consolidation window, rising to ~70% around 2046 as transition revenue sits below its steady-state share, then settling at target. Earlier versions of this figure showed a violent one-year revenue spike and a debt paydown to 50% of GDP — both artifacts of OG-Core's imposed initial wealth distribution, removed by the data-anchored `initial_wealth_ratio` (see the initial household wealth section above).
```
