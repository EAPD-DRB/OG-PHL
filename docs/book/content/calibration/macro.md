...
**The transition follows the government's own consolidation schedule.** A single flat $\alpha_G$ at the identity value would make the model consolidate *immediately* — a primary balance of $-0.7\%$ from the first period, years ahead of the government's plan. Instead, $\alpha_G$ is a declining path pinned to the Medium-Term Fiscal Framework: $[0.1139, 0.1087, 0.1045]$ for 2025, 2026, and 2027 onward. The first two values are the spending shares that reproduce the MTFF's programmed primary balance (the DBCC deficit path less programmed interest: $-2.51$ for 2025, $-2.22$ for 2026) given the model's realized transition revenue under the data-anchored initial wealth; the solved baseline delivers $-2.63$ and $-2.34$. The path reaches the debt-stabilizing level in 2027 and runs 0.6–1.3pp looser than the program in 2027–28: with honest initial wealth, transition revenue dips about 1.7pp of GDP below its steady-state share mid-window, and matching the program's 2028 balance exactly would require cutting $\alpha_G$ below its long-run level and snapping back when the program ends — a V-shaped spending path that would over-fit the enforcement. The MTFF program converging toward the model's debt-stabilizing stance by decade's end remains the fiscal block's broadest validation: the steady state is where the government's plan is headed.
