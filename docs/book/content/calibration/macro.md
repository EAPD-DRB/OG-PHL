...
For the Philippines we use a *centered* form, $r_{gov,DY2}\,(D_t/Y_t - 0.6)^2$ — flat at the 0.60 debt target and steepening only as debt rises away from it — rather than a premium that already bites at the target. This matches the country's experience: it has carried debt in the 40–70% range with modest, stable sovereign spreads (investment-grade since 2013) and saw blowouts only at the extreme debt of the 1980s crisis. We set the convexity to $r_{gov,DY2} = 0.04$; in OG-Core's parameters that is $r_{gov,DY} = -1.2 \times 0.04 = -0.048$, with the shift recentered to $r_{gov,shift} = -0.048$ so the premium is exactly zero at 0.60 and the steady state is unchanged (verified numerically). Note that $D/Y$ is a ratio and $r_{gov}$ a decimal, so the coefficients are not basis points; the premium adds about 36 bp at $D/Y = 0.9$ and 144 bp at 1.2, the marginal sensitivity rising into the range emerging-market studies place at higher debt ([Jaramillo and Weber 2012](https://www.imf.org/external/pubs/ft/wp/2012/wp12198.pdf); [Akitoby and Stratmann 2008](https://www.imf.org/external/pubs/ft/wp/2006/wp0616.pdf); against ~3 bp for advanced economies, [Laubach 2009](https://www.federalreserve.gov/pubs/feds/2003/200312/200312pap.pdf)). A more conservative $r_{gov,DY2} = 0.02$ is a reasonable alternative.

Centering on the target is what makes the premium usable along the transition, not just at the steady state. The multi-industry baseline debt path humps to about 1.2 times GDP at the start of the fiscal-adjustment window (period $t_{G1}$) before returning to 0.60 by around period 80. A premium that bites at the target compounds this into a destabilizing debt-service feedback — higher debt raises $r_{gov}$, which raises interest, which raises debt — pushing the peak to ~1.7 and the transitional sovereign rate to ~16%. The centered form adds only ~140 bp at the hump, leaving the debt peak essentially unchanged (~1.26) and the peak sovereign rate at a plausible ~7.5%. The premium is therefore enabled by default in OG-PHL, where OG-Core's defaults and the other country calibrations leave the debt-elastic channel off.
